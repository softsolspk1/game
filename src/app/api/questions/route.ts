...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        try {
            ...
            console.error("Database Count Check Failed:", e);
        }
        ...
        if (count < 40) {
            try {
                ...
                await prisma.question.createMany({ data: fullSet.map(({ id, ...q }) => q) });
            } catch (e) {
                console.warn("DB Write Refused (Normal on Vercel/SQLite). Falling back to Hardcoded Set.", e);
                // Return difficulty-filtered hardcoded templates instead
                const d = parseInt(difficultyStr || "1");
                const filtered = clinicalTemplates.filter((_, i) => (d === 1 ? i < 25 : i >= 25));
                return NextResponse.json(filtered.map((q, i) => ({ ...q, id: `hard-${i}` })));
            }
        }
        ...
        // If even findMany fails or returns nothing, use hardcoded
        if (!questions || questions.length === 0) {
            const d = parseInt(difficultyStr || "1");
            const filtered = clinicalTemplates.filter((_, i) => (d === 1 ? i < 25 : i >= 25));
            return NextResponse.json(filtered.map((q, i) => ({ ...q, id: `fallback-${i}` })));
        }

        return NextResponse.json(questions);
    } catch (error) {
        console.error("General API GET Error:", error);
        // Ultimate fallback
        const d = parseInt(difficultyStr || "1");
        const filtered = clinicalTemplates.filter((_, i) => (d === 1 ? i < 25 : i >= 25));
        return NextResponse.json(filtered.map((q, i) => ({ ...q, id: `error-fallback-${i}` })));
    }
}
